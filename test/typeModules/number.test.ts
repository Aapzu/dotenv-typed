  ...
  describe('isValidNumberItem', () => {
    ...
    `('$label', ({ values }) => {
      it.each(values as string[])(`returns true for %s`, (value) => {
        expect(validateStringValue(value, Number)).toBe(true)
      })
    })
    ...
    `('$label', ({ values }) => {
      it.each(values as string[])(`returns false for %s`, (value) => {
        expect(validateStringValue(value, Number)).toBe(false)
      })
    })
  })
  ...
  describe('isValidNumberDefaultValue', () => {
    ...
    `('$label', ({ values }) => {
      it.each(values as number[])(`returns true for %s`, (value) => {
        expect(validateValue(value, Number)).toBe(true)
      })
    })
    describe.each`
      label                   | values
    ...
    `('$label', ({ values }) => {
      it.each(values as (string | number | null | undefined)[])(
        `returns false for %s`,
        (value) => {
          expect(validateValue(value, Number)).toBe(false)
        }
      )
    })
  })
